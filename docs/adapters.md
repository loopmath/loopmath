...
| OpenCode | `dev.dagr.adapter.opencode` | `model_usage[].output_tokens` | attempt | integer | Sum of output plus reasoning tokens for the provider/model group (OpenCode counts reasoning apart from output); a message without a reasoning count adds 0. | Core output tokens aggregate the whole attempt. | Field of the proposed repeated model-usage record. |
...
| OpenCode | `dev.dagr.adapter.opencode` | `usage_incomplete_messages.output_tokens` | attempt | integer | Count of assistant messages without a valid nonnegative output-token value, or with a reasoning value that is present but invalid. | OCP cost fields cannot carry per-stream completeness counts. | Child field of the proposed cost-completeness diagnostics record. |
