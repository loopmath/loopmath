...
def cost_terms(st: Structure, piece: str, k: int, setting: Setting, effort: float = 1.0,
    ...
    the source a prediction is for, the effort terms at `effort`, `FitState.effort_for(task)`, and the model's
    price offset from `prices`, `FitState.price_offsets`)."""
    return cost_rest(st, piece, k, setting, source=PREDICT_SOURCE, effort=effort, prices=prices)
...
class Tables:
    ...
    def __init__(self, fs: Any, task: Task, obj: Objective, settings: Sequence[Setting], draw_idx: np.ndarray,
        ...
        self.prices = getattr(fs, "price_offsets", None)  # spec 04 section 2, the price offset per model
    ...
    def _cost_split(self, st: Structure, piece: str, k: int):
        ...
        if base is None:
            base = split_rows([cost_terms(st, piece, 1, s, self.effort, self.prices) for s in self.settings])
            self._memo[("cost_split", id(st), piece, 1)] = base
        if k == 1:
            return base
        parts = base[1]
        case = _minus(cost_terms(st, piece, k, self.settings[0], self.effort, self.prices), Counter(parts[0]))
        check = cost_terms(st, piece, k, self.settings[-1], self.effort, self.prices)
